...
class Notitie(QMainWindow):
    ...
    def opslaan_als(self):
        print("opslaan als")
        try:
            pathname = QFileDialog.getSaveFileName(self, 'Bestand opslaan', OPSLAGLOCATIE, 'Tekst bestanden (*.txt)')
            print(pathname[0])
            filetext = self.plainTextEdit.toPlainText()
    ...
    def open(self):
        print("open")
        try:
            fname = QFileDialog.getOpenFileName(self, 'Open bestand', OPSLAGLOCATIE, 'Tekst bestanden (*.txt);;Alle bestanden (*)')
    ...
    def md_afbeelding(self):
        print("md afbeelding")
        pathname = QFileDialog.getOpenFileName(self, 'Afbeelding openen', OPSLAGLOCATIE, 'Afbeeldingen (*.png *.jpg *.jpeg *.bmp *.gif);;Alle bestanden (*)')
    ...
    def md_link(self):
        print("md link")
        pathname = QFileDialog.getOpenFileName(self, 'Bestand openen', OPSLAGLOCATIE, 'Alle bestanden (*)')
        print(pathname[0]) # gekozen bestand
        if pathname[0]:
            bestandsnaam = pathname[0].split('/')[-1]
            md_code = f"[{bestandsnaam}]({pathname[0]})"
